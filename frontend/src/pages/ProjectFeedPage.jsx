...
const ProjectFeedPage = () => {
  ...
  useEffect(() => {
    ...
    if (skillQueryValue) {
      ...
      if (skillObject) {
        ...
        setFilterSkills([]); // Skill in URL is invalid
      }
    } else {
      setFilterSkills([]); // No skill in URL
    }
    setCurrentPage(pageQuery); // Sync local current page with URL
  ...
  const handleSkillFilterUpdate = (skillsFromModal) => {
    ...
    // setFilterSkills is handled by useEffect when searchParams change
    // setFilterSkills(newSelectedSkill ? [newSelectedSkill] : []); // Can be removed if useEffect handles it
  ...
  const clearSkillFilter = () => {
    // This function now only clears the skill filter and resets the page.
    // The local setFilterSkills state update will be handled by the useEffect
    // when searchParams change, or you can set it here for immediate UI feedback.
    setFilterSkills([]); // Optional: for immediate UI feedback before useEffect runs

    const newSearchParams = new URLSearchParams(searchParams);
    newSearchParams.set('page', '1');
    newSearchParams.delete('skill');
    // Any existing 'search' term will remain.
    setSearchParams(newSearchParams);
  };
  ...
  if (totalPages > 1) {
    ...
    for (let number = 1; number <= totalPages; number++) {
      ...
      } else if (paginationItems.length > 0 && paginationItems[paginationItems.length - 1].key !== 'ellipsis_start' && number < currentPage && currentPage > 4 && paginationItems[paginationItems.length - 1].type !== Pagination.Ellipsis) {
        paginationItems.push(<Pagination.Ellipsis key="ellipsis_start" disabled />);
      } else if (paginationItems.length > 0 && paginationItems[paginationItems.length - 1].key !== 'ellipsis_end' && number > currentPage && currentPage < totalPages - 3 && paginationItems[paginationItems.length - 1].type !== Pagination.Ellipsis) {
          ...
          <div className="text-center py-5">
            ...
            {(filterSkills.length > 0 || searchTermFromURL) &&
              <RBButton
                ...
                onClick={() => {
                  // MODIFIED LOGIC HERE:
                  setFilterSkills([]); // Optional: for immediate UI feedback

                  const newSearchParams = new URLSearchParams(searchParams);
                  newSearchParams.delete('skill');    // Remove skill parameter
                  newSearchParams.delete('search');   // Remove search parameter
                  newSearchParams.set('page', '1');     // Reset page to 1
                  setSearchParams(newSearchParams);   // Single update to URL
                }}
